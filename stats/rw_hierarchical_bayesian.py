"""Rescorla-Wagner モデル 階層ベイズ推定（emcee によるMCMC）

Q学習モデルの階層ベイズ版（q_learning_hierarchical_bayesian.py）と同じ
Non-centered parameterization を採用するが，連合強度 V を「行動」ではなく
「刺激の色（white / black）」に帰属させる点が異なる。
...
def _log_likelihood_single(alpha: float, beta: float,
                            color_choices: np.ndarray, rewards: np.ndarray) -> float:
    """RW モデルの対数尤度（1参加者分）。

    Parameters
    ----------
    color_choices : np.ndarray, dtype int
        1 = white を選択，0 = black を選択
    rewards : np.ndarray, dtype float
        二値化済み報酬（0 or 1）
    """
    v = np.array([0.5, 0.5])  # [white, black]
    ll = 0.0
    for t in range(len(color_choices)):
        dv = np.clip(beta * (v[0] - v[1]), -500, 500)
        p_white = expit(dv)
        a = int(color_choices[t])  # 1=white, 0=black
        ll += np.log((p_white if a == 1 else 1.0 - p_white) + 1e-300)
        v[a] += alpha * (rewards[t] - v[a])
    return ll


def _make_log_posterior(color_choices_list: List[np.ndarray],
                        rewards_list: List[np.ndarray]):
    """クロージャとして対数事後確率関数を生成する。"""
    N = len(color_choices_list)
    ...
    def log_posterior(params: np.ndarray) -> float:
        ...
        # sigma_alpha ~ Half-Cauchy(0,1)（Jacobian 込み）
        ...
        for i in range(N):
            ...
            lp += _log_likelihood_single(
                alpha_i, beta_i, color_choices_list[i], rewards_list[i]
            )
...
def fit_rw_hierarchical_bayesian(
    ...
    for subject, df in concat_list:
        ...
        # chosen_color を 1=white / 0=black に変換
        color_int = (df_clean["chosen_color"] == "white").astype(int).values
        rewards   = (df_clean["reward_points"].values >= 1).astype(float)
        subjects.append(subject)
        color_choices_list.append(color_int)
        rewards_list.append(rewards)
        n_trials_list.append(len(color_int))
    ...
    log_posterior = _make_log_posterior(color_choices_list, rewards_list)
    ...
    # shrinkage 基準：集団レベルの事後平均 alpha・beta

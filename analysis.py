"""
Utility script for inspecting and annotating experiment data.

Steps:
1. Load the raw CSV exported by jsPsych.
2. Keep rows that correspond to actual RDK/response trials (target_group in {white, black}).
3. Drop the first six rows (practice trials).
4. Annotate session / trial indices:
   - num_session: 0..2 assigned to each block of 64 rows (32 trials * 2 rows each).
   - num_trial: within each session, counts 0..31 with two rows per trial (rdk + response).
5. Display the resulting 192-row DataFrame to visually inspect the structure.
"""
...
def concatenate_trials(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for key, group in df.groupby(group_cols, sort=False):
        ...
        base["rotation_angle"] = follow.get("rotation_angle")
...
def analyze_reward_learning_across_subjects(concat_list: List[Tuple[str, pd.DataFrame]] , n_perm: int = 5000) -> pd.DataFrame:
    """
    Run analyze_reward_learning per subject on concatenated datasets and return a combined DataFrame.
    """
    rows = []
    for subj_id, df in concat_list:
        stats_df = analyze_reward_learning(df, n_perm=n_perm)
        stats_df["subject"] = subj_id
...
def main():
    # ---- Single-subject (using DATA_PATH) ----
    df = load_and_prepare(DATA_PATH)
    print("\nConcatenated DataFrame (96 rows expected):")
    print(df.head())
    # df = df.iloc[64:].copy()
    # print(df[["reward_points", "target_group", "dot_color"]])

    plot_reward_by_trial(df, session_id=2, save_path="fig/reward_vs_trial_session_2.pdf")

    session_stats = analyze_reward_learning(df)
    print("\nSession-wise Spearman stats (num_trial vs reward_points):")
    print(session_stats)

    color_stats = analyze_reward_color_dependency(df)
    print("\nTarget color dependency stats (reward_points):")
    print(color_stats)

    after_target_stats = analyze_after_target_dependency(df)
    print("\nAftered (first dot_color) vs Target color dependency (single subject):")
    print(after_target_stats)

    # same_diff_single = compute_same_diff_stats(df)
    # print("\nSame/Different counts (single subject):")
    # print(same_diff_single)

    # Angular error >= 45 counts (on the pre-concatenated, filtered data)

    # trimmed = filter_task_rows(load_data(DATA_PATH))
    # high_err_count = count_high_angular_error(trimmed, threshold=45.0)
    # print(f"\nCount of trials with |angular_error| >= 45 (single subject): {high_err_count}")

    # choice_learning = analyze_target_choice_learning(add_is_target_flag(df))
    # print("\nTarget choice learning per session (logistic is_target ~ num_trial):")
    # print(choice_learning)

    # ---- Multi-subject (all CSVs in data/) ----
    all_data = load_all_concatenated(Path("data"))
    if len(all_data) > 1:
        same_diff_group = analyze_same_diff_across_subjects(all_data)
        print("\nSame/Different bias across subjects:")
        print(same_diff_group)

        after_target_group = analyze_after_target_across_subjects(all_data)
        print("\nAftered vs Target dependency across subjects (CMH):")
        print(after_target_group)

        # combined = combine_subjects(all_data)
        # combined = add_is_target_flag(combined)
        # choice_learning = analyze_target_choice_learning(combined)
        # print("\nTarget choice learning per session (logistic is_target ~ num_trial):")
        # print(choice_learning)

        # reward_learning_multi = analyze_reward_learning_across_subjects(all_data)
        # print("\nReward learning (Spearman num_trial vs reward_points) per subject:")
        # print(reward_learning_multi)

        # mixed_stats = mixed_learning_across_subjects(all_data)
        # print("\nMixed model across subjects: reward_points ~ num_trial + num_session + (1|subject)")
        # print(mixed_stats)

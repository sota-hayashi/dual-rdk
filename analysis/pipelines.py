...
def run_default():
    ...
    # 3) 行動基準で被験者を抽出
    subjects_behavior = get_subjects_by_behavior_data(all_data_learning, threshold=2)

    # 4) 両方の条件を満たす被験者を抽出（AND）
    subjects = subjects_behavior
    ...
    run_behavior(all_data_learning, all_data_awareness)

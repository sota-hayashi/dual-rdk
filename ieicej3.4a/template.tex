...
\begin{figure*}[t]
    ...
    % フェーズ1：固視点の提示（提示時間は1秒）．\\
    % フェーズ2：刺激の提示（提示時間は2秒）．\\
    % フェーズ3：方向の回答（回答時間は無制限）．\\
    % フェーズ4：報酬の提示（提示時間は2秒）．\\
...
全ての参加者は内容に同意した上で実験に参加した．本実験は，大阪大学工学研究科における人を対象とした研究倫理委員会の審査を受けた（承認番号5-7-1）．
...
したがって，練習セッションでは，2つのドット群のドット数の差が明確に異なる．
...
\begin{figure}[t]
    ...
    \caption{本番セッションにおける2種類の刺激パターン\\（上段）白・黒のドット群の運動方向の発生範囲．\\（下段）刺激の具体例（赤矢印はドット群の運動方向を示す）．
...
それぞれの刺激の運動方向の発生範囲は$\pi/4$で，高報酬刺激と低報酬刺激の発生範囲の間隔も$\pi/4$である．
...
上記以外の方向を回答した場合は，ターゲット選択でもディストラクタ選択でもないとみなし，
ターゲット選択率は，ターゲット選択・ディストラクタ選択・
いずれにも該当しない選択のすべてを含む全試行に対する
ターゲット選択の割合として算出した．
...
反応時間の変動は試行難易度の個人差を反映するものではないと判断した．

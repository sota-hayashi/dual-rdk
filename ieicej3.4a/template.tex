...
1試行は，固視点の提示，色（白・黒）の異なる2つのランダムドットキネマトグラム（random dot kinematogram: RDK）刺激の同時提示，方向の回答フェーズ，そして報酬フェーズで構成された．
...
\begin{figure}[t]
    ...
    \caption{本番セッションにおける2種類の刺激パターン\\（上段）白・黒のドット群の運動方向の範囲．\\（下段）刺激の具体例．
...
ディストラクタ刺激方向角度に対する誤差が$\pi/4$未満であればディストラクタ選択とみなした．
...
\subsection{データの除外}
以下の4つの基準を順に適用し，該当する参加者のデータを分析から除外した．
...
この結果は，反応時間の変動が単なるタスクパフォーマンス低下だけでなく，学習過程の初期状態や変化量と関係する可能性を示している．

...
刺激としては，白・黒のドット群がそれぞれ異なる方向に運動する2種類のRDK刺激を用い，それらを同時提示した．
実験参加者は，ドット数の多い方のドット群の運動方向を報告した．
つまり，この課題では，ドット数の多い刺激がターゲット刺激であり，ドット数の少ない刺激がディストラクター刺激であった．
参加者には，各試行において，キーボードの右矢印キーと左矢印キーを操作し，バーを回転しドット群の運動方向に合わせるように指示した．
バーの初期位置はランダムに決定され，回答時間は無制限であった．
回答後は，ターゲット刺激に対する角度誤差に応じて報酬（報酬の決定の仕方については\ref{sec:reward_structure}節で述べる）を与えた．
...
つまり，2つのRDK刺激のドット数は同数であるため，ターゲット刺激とディストラクター刺激の区別はない．
ただ，ドット群が運動する方向は2色のドット群でそれぞれ異なり，その運動方向に応じて報酬が変化するように設定した．
要するに，本番セッションでは，参加者はドット数が多いと感じた方のドット群の運動方向を報告することを指示されたが，
...
\begin{figure}[t]
    \centering
    \includegraphics[width=1.0\linewidth]{../fig/learning-stimuli-modified.pdf}
...
左側のペアでは，高報酬刺激は黒色ドット群であり，低報酬刺激は白色ドット群である．
逆に右側のペアでは，高報酬刺激は白色ドット群であり，低報酬刺激は黒色ドット群である．
各試行では，これら2種類の刺激のペアのうちどちらかがランダムに選択されて提示された．
それぞれの刺激の運動方向の範囲は$\pi/4$で，高報酬刺激と低報酬刺激の間隔は$\pi/4$である．
...
ターゲット刺激方向角度に対する回答角度の誤差が$\pi/4$未満であればターゲット選択とみなし，
ディストラクター刺激方向角度に対する誤差が$\pi/4$未満であればディストラクター選択とみなした．
